...
# current enclave = 0x7fad1c5d1032fcdc3f8990a4d7c25d89c023ec47d31fe2ac4e5a65e0a9b199bd
...
case "$VERSION_CONTROL" in
    "update")
        ...
        # Check if OLD_ENCLAVE_ID is provided in config.json
        OLD_ENCLAVE_ID=$(jq -r '.OLD_ENCLAVE_ID // empty' config.json)
    ...
    "destroy_only")
        ...
        if [ -z "$OLD_ENCLAVE_ID" ]; then
            echo "Error: OLD_ENCLAVE_ID not found in config.json"
            exit 1
        fi
