...
# Python dependencies removed - verification backend now runs externally
...
WORKDIR /src/attestation-backend

# Clean environment - Redis-based build (no Kafka dependencies)
...
# Sui CLI calls will be made via host proxy - no need for CLI in container
RUN cp /target/${TARGET}/release/init initramfs/
RUN cp /src/attestation-backend/target/${TARGET}/release/attestation_server initramfs/
RUN cp /src/attestation-backend/run.sh initramfs/

# Copy environment configuration files (.env only approach)
# Note: Environment variable precedence in container:
# 1. .env files - HIGHEST priority
# 2. Default values in code - LOWEST priority
RUN cp /src/attestation-backend/.env initramfs/attestation_server.env 2>/dev/null || echo "# Rust service env file" > initramfs/attestation_server.env
